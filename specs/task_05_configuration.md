...
## Scope
1. Create `src/config/` module providing strongly-typed config structs.
...
## 1. Module Layout
- `src/config/mod.rs` exposing `Config::load()` and structs.
- Modules:
  - `src/config/sources.rs` — load from file/env.
  - `src/config/validation.rs` — cross-field checks.
  - `src/config/models.rs` — struct definitions.
  - `src/config/watch.rs` (optional) — file watcher for reload.
...
- `ServerConfig { bind_addr: SocketAddr, api: ApiLimits (max_payload_bytes), fjall_path: PathBuf }`
...
- `Config::load()`:
  ...
  4. Apply defaults (e.g., `server.api.max_payload_bytes` defaults 5 MB).
...
- Validate `server.api.max_payload_bytes` ≤ 5 MB (per spec) unless explicitly overridden.
...
## 7. Deliverables
- `src/config/` module with load/validate functions, resolved proxy helper, tests for parsing/validation.
