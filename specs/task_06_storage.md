...
## 1. Module Layout
- `src/storage/mod.rs` exports `StorageClient`.
- `src/storage/config.rs` – parse storage config (provider, endpoint, bucket, credentials).
- `src/storage/client.rs` – actual implementation using Arrow `object_store` crate (S3 backend).
- `src/storage/manifest.rs` – helpers for writing/reading manifest files.
- `src/storage/resource.rs` – uploading resources with metadata.
- `src/storage/errors.rs` – error enum mapping S3/object store errors into FetchBox codes.
- `src/storage/util/checksum.rs` – optional SHA-256 computation.
...
## 8. Deliverables
- `src/storage/` module with APIs above, tests, and docs.
